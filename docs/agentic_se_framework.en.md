...
## 0. Background: Between the Two Poles of AI Agent-Driven Software Engineering

Over the past year, the rise of AI agents taking on real software engineering work has become one of the most heated — and most contested — topics in the industry. Two opposing narratives are in circulation at the same time:

- **The optimists** claim that large models drive the marginal cost of "writing code" toward zero, accelerating software development by orders of magnitude, and that an individual or a small team can now build what used to require a whole engineering team.
- **The pessimists** argue that large models, trained on the statistical patterns of public code, can quickly stack a "looks-like-it-works" prototype for small projects, but once they hit real-world engineering complexity — multiple modules, long-lived evolution, team collaboration — they rapidly produce a "spaghetti code mountain": style drift, missing seams, leaking boundaries, zero testability. The result is software that cannot be maintained and cannot evolve.
...
The point is not that "the order must be one-size-fits-all", but that every phase must answer one clear engineering question and leave behind the corresponding artifacts. The diagram below stacks five horizontal bands — core engineering goals / engineering mainline phases / execution skills / artifacts / auxiliary cross-cutting skills — on a single canvas:

- **Core engineering goals** (dark-gray band): the engineering question each phase must answer.
- **Engineering mainline phases** (purple band): six continuous activities from bootstrap through operation & evolution.
- **Execution skill line** (teal band): the concrete agent skill for each phase.
- **Artifact line** (blue band): the auditable artifacts each skill produces.
- **Auxiliary / cross-cutting skills** (amber footer band): the on-demand toolbox skills.

Phases 3 (specification & decomposition) and 4 (iteration loop) are wrapped by the green dashed rectangle that holds `impl-loop` — making explicit that it is not a sub-skill of any single phase, but the end-to-end delivery engine that spans Phases 3 -> 4. The engine is unpacked in its own section 3.6.
...
The diagram above stacks the five bands — core engineering goals, engineering mainline phases, execution skills, artifacts, and auxiliary/cross-cutting skills — on a single canvas; the phase 0–5 mainline-to-skill correspondences are repeated in table form (see the "mainline phase-to-skill mapping" at the top of Chapter 2), so you can look them up by phase while reading the chapters that follow.
