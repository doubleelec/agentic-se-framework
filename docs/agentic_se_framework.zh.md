# 基于 AI Agent 的软件工程实施方案

<small>**最后修改时间**：2026-09-01</small>

<details>
<summary><small>维护信息</small></summary>

<ul>
  <li>Source：<code>docs/agentic_se_framework.zh.md</code></li>
  <li>任何对内容、标题编号或 SVG 几何的修改，请同步更新英文版；<code>ops/check_bilingual.ps1</code> 会校验 heading 对齐，<code>ops/refresh_panorama.ps1 -Check</code> 会校验 SVG 同步。</li>
</ul>
</details>

## 0. 缘起：在 AI Agent 主导软件工程的两极之间 {#ch-0}

过去一年，AI Agent 接管部分软件工程活动的趋势成为业界最热也最具争议的话题之一，两种截然相反的论调同时流行：

- **乐观派**：大模型让"写代码"的边际成本趋近于零，软件开发速度可以提升数十倍；个人或小团队就能撬动过去需要整支团队才能完成的产品。
- **悲观派**：大模型受训于公共代码的统计学规律，对小项目可以快速堆出"看起来能跑"的原型，但一旦进入真实业务的复杂工程——多模块、长期演进、团队协作——它就会迅速产出"屎山代码"：风格漂移、接缝缺失、边界渗漏、可测试性归零，最终既不能维护也无法演进。

两种说法都有真实的工程证据支撑，也都有可证伪的反例。它们并非互斥，而是揭示了同一种范式的两个剖面：**大模型是前所未有的"代码生成器"，但它不是一个天生的"软件工程师"。** 当需求被合理地拆解、边界被预先设计、每一步产出都被显式工件约束时，它确实可以十倍量级交付；当这些前提缺失时，它就会以人类程序员相同的速度、甚至更快的速度制造技术债。

本方案的核心命题是：**用经典软件工程思想给大模型套上受控的缰绳，让它在严格的工件、边界、审查与验证体系内生产代码，并使整条流水线对人类完全可视化、可审计、可回滚。**

由此出发，本方案主张三条总原则：

1. **以工程思想为骨，以大模型为肌肉**。领域语言、架构决策、质量属性场景、ADR、TDD 闸门、架构边界 TOML 约束——SWEBOK / ISO 12207 / 42010 体系内的成熟实践不是"老旧包袱"，而是让 AI 输出可维护、可演进、可验证产品的必要骨架。
2. **可视化优先**。每一阶段的产出（`CONTEXT.md`、`ADR`、`docs/architecture.md`、`docs/action-plan.md`、`spec.md`、工单 DAG、`failsafe` 快照）都是人类可读、可 diff、可审阅的活文档，而不是隐藏在 prompt 里的隐式状态。
3. **可检查与可回滚**。测试闸门、架构边界校验、双轴 `code-review`、分阶段快照比对共同构成可证伪的验收闭环——任何一步不通过都可被立刻定位、立刻回退，而不是等到"屎山堆完才发现"。

本方案采用工程受控路线：把乐观派的"加速"目标与悲观派指出的"失控"风险放进同一份可执行方案，分别用流程提速与闸门熔断来同时应对。
...
要点不在于“顺序必须一刀切”，而在于每个阶段都必须回答一个明确的工程问题，并留下相应工件。下图把核心工程目标、工程主阶段、执行技能线、产物线、辅助/横切技能五条横带叠在同一画布上：

- **核心工程目标**（深灰行）：每个主阶段必须回答的工程问题。
- **工程主阶段**（紫色行）：从启动到运行演进的六个连续活动。
- **执行技能线**（青色行）：每个阶段对应的具体 Agent 技能。
- **产物线**（蓝色行）：每个技能产出的可审计工件。
- **辅助/横切技能**（底部横条）：按需外挂的工具箱技能。

阶段 3（规格与拆解）与阶段 4（迭代闭环）外层的绿色大虚线框整体包裹着 `impl-loop`，强调它不是任一阶段的子技能，而是阶段 3→4 的端到端交付编排引擎——这一点在 3.6 节独立展开。
...
上图把核心工程目标、工程主阶段、执行技能线、产物线、辅助/横切技能五条横带叠在同一画布上；阶段 0–5 的主线—技能对应关系在表格形式里重复给出（见第 2 章开头的"主线阶段—技能映射"），便于在阅读后续章节时按阶段反查。

## 3. 操作手册：按阶段详细指导 {#ch-3}

以下是软件工程生命周期各个阶段的详细操作指南，包括每个阶段应该使用什么技能、如何使用以及预期产出的工件。

### 3.1 阶段 0：启动（协作底座） {#sec-3-1}
...
### 3.6 跨阶段交付编排引擎：[`impl-loop`](#impl-loop) {#sec-3-6}
